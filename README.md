...
- **泛职业文档分析**：把简历能力泛化为文档理解、结构提取、评估建议、报告导出。
- **多场景情景模拟**：把单一面试场景泛化为面试、答辩、咨询、知识问答、角色扮演等多轮互动。
...
这也是 `Ruici` 与原始 `InterviewGuide` 的本质区别：`InterviewGuide` 是单领域练习型项目，`Ruici` 是可扩展的多场景 AI 平台。
