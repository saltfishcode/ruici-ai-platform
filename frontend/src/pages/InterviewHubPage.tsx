  ...
  return (
    <div className="max-w-5xl mx-auto">
      ...
      <div className="mb-8">
        ...
        <p className="text-stone-500 dark:text-[#9ca3af] mt-1">选择模拟模式、情景方向与岗位方向，快速开始练习</p>
      </div>

      {/* 配置区域 */}
      <div className="bg-white dark:bg-[#1a1f2e] rounded-2xl border border-stone-200 dark:border-[#2d3548] p-6 mb-8">
        <div className="space-y-6">
          {/* 面试模式 */}
          <div>
            <p className="flex items-center gap-2 mb-3 text-sm font-semibold text-primary-700 dark:text-[#e5e7eb]">
              模拟模式
            </p>
                ...
                return (
                  <button
                    ...
                    className={`flex items-center gap-3 p-4 rounded-xl border transition-all duration-200 text-left
                      ${selected
                        ? 'border-primary-800 bg-stone-100 dark:bg-[#1f2937] dark:border-white'
                        : 'border-stone-200 dark:border-[#2d3548] bg-white dark:bg-[#1a1f2e] hover:border-stone-300 dark:hover:border-[#4b5563]'
                      }`}
                  >
                    <Icon className={`w-6 h-6 flex-shrink-0 ${selected ? 'text-primary-700 dark:text-[#f3f4f6]' : 'text-stone-400'}`} />
                    <div className="min-w-0">
                      <p className={`font-semibold text-sm flex items-center gap-2 ${selected ? 'text-primary-800 dark:text-[#f3f4f6]' : 'text-primary-800 dark:text-[#f3f4f6]'}`}>
                        <span>{opt.label}</span>
                        {opt.recommended && (
                          <span className="px-1.5 py-0.5 rounded-full text-[10px] font-semibold bg-green-100 text-green-700 dark:bg-green-900/40 dark:text-green-300">
                            推荐
                          </span>
                        )}
                      </p>
                      <p className="text-xs text-stone-500 dark:text-[#9ca3af]">{opt.desc}</p>
                    </div>
                  </button>
                );
              })}
              ...
              {SIMULATION_DIRECTION_OPTIONS.map(option => {
                ...
                return (
                  ...
                  >
                    ...
                    <p className="mt-1 text-xs leading-5 text-primary-400 dark:text-[#9ca3af]">
                      {option.desc}
                    </p>
                  </button>
                );
                  ...
                  return (
                    ...
                    >
                      <div className={`w-8 h-8 rounded-lg flex items-center justify-center text-sm flex-shrink-0 ${
                        ...
                        {IconComponent
                          ? <IconComponent className={`w-4 h-4 ${selected ? (skill.display?.iconColor || 'text-primary-600') : 'text-primary-400 dark:text-[#9ca3af]'}`} />
                ...
                >
                  <div className={`w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 ${
                    ...
                    {(() => {
                      ...
                      return CustomIcon
                        ? <CustomIcon className={`w-4 h-4 ${config.isCustomSkill ? 'text-primary-600 dark:text-[#9ca3af]' : 'text-primary-400 dark:text-[#9ca3af]'}`} />
                        : <span className="text-sm">✨</span>;
                    })()}
                  </div>
                  <span className={`text-xs font-medium ${config.isCustomSkill ? 'text-primary-700 dark:text-[#9ca3af]' : 'text-primary-400 dark:text-[#9ca3af]'}`}>
                    自定义 JD
                  </span>
                </button>
              </div>
                ...
                return (
                  ...
                  >
                    <p className={`text-sm font-semibold ${selected ? 'text-primary-700 dark:text-[#9ca3af]' : 'text-primary-600 dark:text-[#d1d5db]'}`}>
                      ...
                      <p className="text-xs text-primary-300">{getDifficultyDescription(config.simulationDirection, opt.value)}</p>
                    </button>
                  );
                })}
            </div>
                ...
                <div className="bg-gradient-to-br from-primary-50/80 to-blue-50/80 dark:from-primary-900/20 dark:to-blue-900/10 rounded-xl p-4 border border-primary-100 dark:border-[#2d3548]/30">
                  ...
                  <select
                    ...
                    className="w-full px-4 py-2.5 rounded-lg border border-primary-200 dark:border-[#2d3548]/50
                      bg-white dark:bg-[#1f2937] text-sm text-primary-800 dark:text-[#f3f4f6] disabled:opacity-60 disabled:cursor-not-allowed
        ...
        {/* 开始面试按钮 */}
        <div className="mt-6 pt-6 border-t border-stone-100 dark:border-[#2d3548]">
          <motion.button
            type="button"
            onClick={handleStart}
            ...
            className="w-full px-6 py-3.5 rounded-[32px] font-semibold text-sm transition-all
              bg-primary-800 dark:bg-[#1f2937] hover:bg-primary-700 dark:hover:bg-stone-100
              text-white dark:text-[#f3f4f6] shadow-soft hover:shadow-medium disabled:opacity-50 disabled:cursor-not-allowed"
          >
      ...
      {/* 最近面试记录 */}
      <div className="bg-white dark:bg-[#1f2937] rounded-2xl shadow-sm border border-stone-100 dark:border-[#2d3548] p-6">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-lg font-bold text-primary-700 dark:text-[#f3f4f6]">最近模拟记录</h2>
          <Link
            to="/app/simulation/history"
        ...
        {loadingRecent ? (
          ...
          <div className="text-center py-10">
            <p className="text-primary-300 dark:text-[#9ca3af] text-sm">暂无模拟记录，选择方向开始第一次练习吧</p>
          </div>
        ) : (
            ...
            {recentInterviews.map((item, index) => {
              ...
              return (
                <motion.div
                  ...
                  onClick={() => {
                    if (item.type === 'text') {
                      navigate(`/app/simulation/history/${item.id}`);
                    } else if (item.voiceSessionId) {
                      navigate(`/app/voice/${item.voiceSessionId}/evaluation`);
                    }
                  }}
                  className="flex items-center gap-4 p-4 rounded-xl hover:bg-stone-50 dark:hover:bg-[#374151]/50 transition-colors cursor-pointer group"
                >
                  {/* 类型图标 */}
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      <span className="font-medium text-sm text-primary-700 dark:text-[#f3f4f6] truncate">{item.title}</span>
                    ...
                    <div className="flex items-center gap-3 mt-1">
                      <span className="text-xs text-primary-300 dark:text-[#9ca3af]">
                      ...
                      {isCompleted && item.overallScore !== null && (
                        <span className="text-xs text-primary-500 dark:text-[#d1d5db]">
                  ...
                  {/* 箭头 */}
                  <svg className="w-4 h-4 text-primary-200 dark:text-[#9ca3af] group-hover:text-primary-500 group-hover:translate-x-0.5 transition-all flex-shrink-0" viewBox="0 0 24 24" fill="none">
                    <title>查看详情</title>
