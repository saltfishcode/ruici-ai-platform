package com.ruici.ai.modules.schedule.service;
...
/**
 * 统一的场景邀约解析服务，整合规则解析和 AI 解析。
 *
 * <p>当前最常见输入仍然是面试邀约，但这里的目标已经扩展为“通用排期文本结构化”。
 * 为了兼容现有前后端契约，结构化结果暂时继续沿用 `interviewTime` / `interviewType`
 * 等历史字段名。</p>
 */
@Slf4j
@Service
...
public class InterviewParseService {
    ...
    private static final String PARSE_PROMPT = """
        你是一个专业的场景邀约信息提取助手。请仔细分析以下文本，提取排期相关信息。

        **提取规则**：
        1. companyName（关联主体）：提取公司、机构、团队或活动主办方名称，**必需字段**
        2. position（关联主题）：提取岗位、议题、活动主题或职位名称，**必需字段**
        3. interviewTime（兼容旧字段名，实际表示开始时间）：提取开始时间并转换为 ISO 8601 格式，**必需字段**
           - 格式：YYYY-MM-DDTHH:MM:SS（例如：2026-04-10T14:00:00）
           - 若只有相对时间（如"明天下午2点"），根据当前日期 %s 推算
        4. interviewType（兼容旧字段名，实际表示交互形式）：ONSITE（现场）/ VIDEO（视频）/ PHONE（电话）
        5. meetingLink（链接或会议信息）：提取完整的会议链接或会议号+密码
        6. roundNumber（轮次）：提取数字（1-10），如"二面"提取为2；若无轮次可省略
        7. notes（其他备注）：包含联系人姓名（如果不重要可忽略）、时长（**默认30分钟**）等。

        **重要提示**：
        - 联系人是谁不重要，只需在 notes 中提及。
        - 优先保证 companyName、position、interviewTime 的准确性。
        - 如果文本中没说时长，默认设置为 30 分钟。
        - 当前返回 JSON 的键名必须保持兼容，不要擅自改成 startTime 或 sceneType。
        ...
        **返回格式**：
        ...
        {"companyName":"阿里巴巴","position":"Java工程师","interviewTime":"2026-04-10T14:00:00","interviewType":"VIDEO","meetingLink":"https://meeting.feishu.cn/xxx","roundNumber":2,"interviewer":"张三","notes":"线上沟通"}
        """;
     ...
     * @return Parse response with extracted schedule details
     */
