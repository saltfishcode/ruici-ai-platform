 ...
 * 语音交互配置属性。
 *
 * <p>当前配置前缀仍为 `app.voice-interview`，这是为了兼容现有环境变量与部署脚本。
 * 但在业务语义上，这组配置已经服务于整个 `voice` 模块，而不再只代表语音面试。</p>
 */
@Data
@Component
...
public class VoiceInterviewProperties {
    ...
    /**
     * 单轮 AI 回复最大字符数（超出会截断到句子边界）。
     *
     * <p>虽然旧注释里写的是“面试官回复”，但该限制现在适用于整个语音交互模块。</p>
     */
    ...
    public static class OpeningConfig {
        ...
        private String algorithmQuestion =
            "你好，我是本场语音场景助手。先做一道算法与数据结构热身题：请你从“哈希表/堆/栈/队列/树/图”里选两个，结合一道你熟悉的题，口述“为什么选这个结构、核心步骤、时间复杂度、空间复杂度、边界条件与反例”。本场不需要写代码，重点看你的思路和取舍。";
        private String backendQuestion =
            "你好，我是本场语音场景助手。第一个问题：请用 1 分钟介绍一个你深度参与的项目，按三点回答：业务目标、你负责的核心模块、核心技术栈。说完我会立刻追问一个关键技术决策。";
    }
}
