...
public class ResumePersistenceService {
     ...
     */
    @Transactional(rollbackFor = Exception.class)
    public ResumeAnalysisEntity saveAnalysis(ResumeEntity resume, DocumentAnalysisResponse analysis) {
        try {
            // 使用 MapStruct 映射基础字段
    ...
    public Optional<ResumeAnalysisEntity> getLatestAnalysis(Long resumeId) {
        return Optional.ofNullable(analysisRepository.findFirstByResume_IdOrderByAnalyzedAtDesc(resumeId));
    }
    
    /**
     * 获取简历的最新评测结果（返回DTO）
     */
    public Optional<DocumentAnalysisResponse> getLatestAnalysisAsDTO(Long resumeId) {
    ...
    public List<ResumeAnalysisEntity> findAnalysesByResumeId(Long resumeId) {
        return analysisRepository.findByResume_IdOrderByAnalyzedAtDesc(resumeId);
    }
    
    /**
     * 将实体转换为DTO
     */
    public DocumentAnalysisResponse entityToDTO(ResumeAnalysisEntity entity) {
        try {
            ...
            List<Suggestion> suggestions = objectMapper.readValue(
    ...
    public void deleteResume(Long id) {
        ...
        // 1. 删除所有简历分析记录
        List<ResumeAnalysisEntity> analyses = analysisRepository.findByResume_IdOrderByAnalyzedAtDesc(id);
        if (!analyses.isEmpty()) {
